...
export async function createAllTables() {
  if (!dbPool) {
    logger.error('NeonDB Pool não disponível. Tabelas não criadas.');
    return;
  }

  const sql = `
    -- Tabela de Colaboradores (Central)
    ...
    CREATE TABLE IF NOT EXISTS sectors (
      ...
      best_employee_id INTEGER REFERENCES employees(id),
      created_at TIMESTAMP DEFAULT NOW()
    );
    ...
    -- Garante que colunas essenciais existam (migração incremental)
    DO $$ 
    BEGIN 
      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='sector_id') THEN
        ALTER TABLE employees ADD COLUMN sector_id INTEGER;
      END IF;
      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='salary') THEN
        ALTER TABLE employees ADD COLUMN salary NUMERIC(12,2) DEFAULT 0;
      END IF;
      ...
      END IF;
      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='registration') THEN
        ALTER TABLE employees ADD COLUMN registration VARCHAR(20);
      END IF;
      ...
      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='status') THEN
        ALTER TABLE employees ADD COLUMN status VARCHAR(30) DEFAULT 'Ativo';
      END IF;
      IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='user_role') THEN
        ALTER TABLE employees ADD COLUMN user_role VARCHAR(20) DEFAULT 'EMPLOYEE';
      END IF;
    END $$;
  ...
  try {
    ...
    logger.info('NeonDB: Todas as tabelas foram criadas/verificadas com sucesso.');
  } catch (err) {
    logger.error('NeonDB: Erro ao criar tabelas:', err);
  }
}
